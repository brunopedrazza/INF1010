...
int* menoresCaminhos (Grafo *grafo, int no_inicial){
	...
	for (i=0 ; i<grafo->nv ; i++) { 
		...
		ultimomenor[i] = -2; /* para indicar erro */
	}
	...
	for (i=0 ; i<grafo->nv ; i++) {
      printf ("\nverifica caminhos via nó %d\n", no);
      debug_listap_show (prio, "lista atual:");
		while (aux != NULL){
			if (!marcados[aux->noj]){
               ...
               printf ("trocou a distancia até o nó %d por %d\n", aux->noj, peso);
               debug_listap_show (prio, "lista corrigida:");
               printf("\n");
